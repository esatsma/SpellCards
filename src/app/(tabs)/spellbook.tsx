...
export default function SpellBook() {
  ...
  const spellBookSpells =
    data?.results?.filter((spell) => spells.includes(spell.index)) || [];
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <View style={styles.container}>
        {spellBookSpells.length === 0 ? (
          ...
          <SpellList spells={spellBookSpells} />
        )}
      </View>
    </SafeAreaView>
  );
...
const EmptyScreen = () => {
  return (
    <View style={styles.emptyList}>
      <Text size={"2xl"}>
        You haven't saved any spells to your Spellbook yet. To do so, go to the
        <Link href={{ pathname: "/(tabs)" }} style={styles.link}>
          Spells screen
        </Link>{" "}
        and press the <Feather size={28} name="book-open" color={"#06402B"} />
        -icon
      </Text>
    </View>
  );
